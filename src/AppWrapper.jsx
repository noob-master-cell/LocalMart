// src/AppWrapper.jsx - Updated with error boundary and store
import React, { useState, useCallback, useEffect } from "react";
import { BrowserRouter } from "react-router-dom";
import AppLayout from "./AppLayout.jsx";
import MessageBar from "./components/UI/MessageBar.jsx";
import ErrorBoundary from "./components/UI/ErrorBoundary.jsx";
import useAuth from "./hooks/useAuth.jsx";
import useAppStore from "./store/appStore.js";
import optimizedFirebaseService from "./services/firebaseService.js";
...
function AppWrapper() {
  ...
  // Handle various app-level events
  useEffect(() => {
    ...
    const handlePerformanceIssue = () => {
      if (process.env.NODE_ENV === "development") {
        ...
        if (health.cacheSize > 800) {
          console.warn("Large cache size detected, consider cleanup");
        }
      }
    };

    // Item operation success handler
    ...
    const perfInterval = setInterval(handlePerformanceIssue, 30000); // Every 30 seconds
  ...
  // Cleanup Firebase connections on unmount
  useEffect(() => {
    return () => {
  ...
  return (
    <ErrorBoundary>
      <BrowserRouter>
        ...
        {/* Enhanced MessageBar with all features */}
        <MessageBar
        ...
        {/* App Install Banner (for PWA) - Only show if not already installed */}
