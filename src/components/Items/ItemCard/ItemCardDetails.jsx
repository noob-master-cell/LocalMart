// src/components/Items/ItemCard/ItemCardDetails.jsx
 ...
 * Now prioritizes displayName over email for showing the poster's name.
 */
const ItemCardDetails = ({
  ...
  // Enhanced poster name logic - prioritize displayName, fallback to email, then "Anonymous"
  const getDisplayName = () => {
    if (item.userDisplayName && item.userDisplayName.trim()) {
      return item.userDisplayName.trim();
    }
    if (item.userEmail) {
      // Extract name part from email if no display name is available
      const emailName = item.userEmail.split("@")[0];
    ...
    <div className={`p-4 flex-grow flex flex-col`}>
      <div className="flex-grow">
        ...
        <div className="space-y-1 mb-4">
          ...
          <p className="text-xs text-gray-400 truncate" title={displayName}>
